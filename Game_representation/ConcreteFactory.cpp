...
shared_ptr<PlayerModel> ConcreteFactory::createPlayer() {
    ...
    ownType::Position leftUpper(-1, -1 + 26.f / 272.f);
    ownType::Position rightDown(-1 + 26.f / 272.f, -1);

    player->setTileType(ownType::PLAYER);
    ...
    shared_ptr<ownView::View> playerObserver =
        make_shared<PlayerView>(player->getLeftUpperCorner(), ownType::PLAYER, sfWindow);
    // add the observer to the playerModel
    player->addObserver(playerObserver);
    return player;
}
shared_ptr<GoalModel> ConcreteFactory::createGoal(ownType::inputRectangles tile) {
    shared_ptr<GoalModel> goal = make_shared<GoalModel>();

    goal->setTileType(ownType::GIRL);
    ...
    shared_ptr<ownView::View> goalObserver = make_shared<GoalView>(goal->getLeftUpperCorner(), ownType::GIRL, sfWindow);
    goal->addObserver(goalObserver);
    return goal;
}
shared_ptr<WallModel> ConcreteFactory::createWall(ownType::inputRectangles tile) {
    ...
    shared_ptr<ownView::View> wallObserver =
        make_shared<WallView>(wall->getLeftUpperCorner(), wall->getTileType(), sfWindow);
